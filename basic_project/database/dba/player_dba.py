...
from database.dbo.player_dbo import PlayerDBO as Player
...
class PlayerDBA(MongoDBA):
    ...
    def __find_one(self, condition: Dict[str, Any], session=None) -> Player:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when find one: {err}")
            return None
    ...
    def __find_many(
        ...
        try:
            ...
            return [Player.from_json_obj(data) for data in cursor]
        except ValueError as err:
            Logger("PlayerDBA").log_error(f"Error when find many: {err}")
            return None

    def __find_by_id(self, id, session=None) -> Player:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when find by id: {err}")
            return None
    ...
    def __find_by_ids(self, ids: List[Any], session=None) -> List[Player]:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when finding by ids: {err}")
            return []

    def __insert_one(self, obj: Player, session=None) -> ObjectId:
        try:
            data = obj.model_dump(exclude_defaults=True)
            result = self.collection.insert_one(data, session=session)
            return result.inserted_id
        except ValueError as err:
            Logger("PlayerDBA").log_error(f"Error when insert: {err}")
            return None

    def __insert_many(self, objs: List[Player], session=None) -> List[ObjectId]:
        try:
            data = [obj.model_dump(exclude_defaults=True) for obj in objs]
            result = self.collection.insert_many(data, session=session)
            return result.inserted_ids
        except ValueError as err:
            Logger("PlayerDBA").log_error(f"Error when insert many: {err}")
            return None
    ...
    def __update_many(
        ...
        try:
            ...
            print(err)
            Logger("PlayerDBA").log_error(f"Error when update many: {err}")
            return False
    ...
    def __update_by_id(self, id, new_value: List[Any], session=None) -> bool:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when update by id: {err}")
            return False
    ...
    def __delete_one(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when delete one: {err}")
            return False
    ...
    def __delete_many(self, condition: Dict[str, Any], session=None) -> bool:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when delete many: {err}")
            return False
    ...
    def __delete_by_id(self, id, session=None) -> bool:
        try:
            ...
            Logger("PlayerDBA").log_error(f"Error when delete by id: {err}")
            return False
    ...
    def find_many(self, condition: Dict[str, Any], n: int = None) -> List[Player]:
        result = self.transaction(self.__find_many, condition=condition, n=n)
        return result

    def find_by_id(self, id) -> Player:
        result = self.transaction(self.__find_by_id, id=id)
        return result

    def find_by_ids(self, ids: List[Any]) -> List[Player]:
