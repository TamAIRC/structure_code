...
class QuestionDBA(MongoDBA):
    ...
    def update_one(self, id: ObjectId, new_value: Dict[str, Any], session=None) -> bool:
        try:
            normalized_id = normalize_id(id)
    ...
    # Cần chỉnh sửa
    def __delete_questions(self, ids: List[ObjectId], session=None):
        try:
    ...
    def delete_questions(self, ids: List[ObjectId]):
        delete_q = self.transaction(self.__delete_questions, ids=ids)
        return delete_q
